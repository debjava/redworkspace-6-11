...
/**
 * This class is used to generate a dtd from the xml file path 
 * @author Debadatta Mishra (PIKU)
 * Original Author of DTD Generator :  M.H.Kay
 * For more details refer to the site : http://saxon.sourceforge.net/dtdgen.html	
 */
public class MyDTDGenerator extends org.xml.sax.helpers.DefaultHandler implements DTDGenerator
{
	...
	// object    
	private StringBuilder dtdBuilder = new StringBuilder("");

	public MyDTDGenerator () 
	{
	...
	@Override
	public String generateDTDfromXML(File xmlFile) throws DtdGenerationException
	{
		String dtdString = null;
		try 
		{
			InputSource is = new InputSource(xmlFile.toURL().toString());
			XMLReader parser = SAXParserFactory.newInstance().newSAXParser().getXMLReader();
			parser.setContentHandler(this);
			parser.parse(is);
			printDTD();
			dtdString = dtdBuilder.toString();
		}
		catch( NullPointerException npe )
		{
			throw new DtdGenerationException("Exception while generating the DTD from XML. \n"+npe.getMessage());
		}
		catch (Exception e) 
		{
			throw new DtdGenerationException("Exception while generating the DTD from XML. \n"+e.getMessage());
	...
	public String generateDTDfromXML(String xmlContents) throws DtdGenerationException
	{
		...
		try 
		{
			if( xmlContents == null ) throw new NullPointerException();
			bis = new ByteArrayInputStream(xmlContents.getBytes());
			InputSource source = new InputSource( bis );
			if( xmlContents == null || xmlContents.length() == 0 ) throw new NullPointerException();
			XMLReader parser = SAXParserFactory.newInstance().newSAXParser().getXMLReader();
			parser.setContentHandler(this);
			parser.parse(source);
			printDTD();
	...
	private void printDTD ()
	{
		...
		while ( e.hasNext() )
		{
			...
			if (childKeys.size()==0 && !ed.hasCharacterContent)
			{
				...
				dtdBuilder.append("<!ELEMENT " + elementname + " EMPTY >\n");
			...
			if (childKeys.size()==0 && ed.hasCharacterContent)
			{
				...
				dtdBuilder.append("<!ELEMENT " + elementname + " ( #PCDATA ) >\n");
			}

			//ELEMENT content
			if (childKeys.size()>0 && !ed.hasCharacterContent) {
				//System.out.print("<!ELEMENT " + elementname + " ( ");
				dtdBuilder.append("<!ELEMENT " + elementname + " ( ");
				if (ed.sequenced) {
					...
					while (c1.hasNext()) {
						...
						dtdBuilder.append((String)c1.next());
						if (c1.hasNext())
						{
							//System.out.print(" | ");
							dtdBuilder.append(" | ");
			...
			if (childKeys.size()>0 && ed.hasCharacterContent) {
				...
				while (c2.hasNext()) {
					...
					dtdBuilder.append(" | " + (String)c2.next());
				}
				//System.out.print(" )* >\n");
			};
			...
			while ( a.hasNext() )
			{
				...
				if (isid) { 
					...
					while (v.hasNext()) {
						...
						dtdBuilder.append("(String) v.next()");
						if (!v.hasNext()) break;
						//System.out.print(" | ");
						dtdBuilder.append(" | ");
					};
				...
				if (!isfixed) {
					if (required)
					{
						...
						dtdBuilder.append(" #REQUIRED >\n");
					}
					else
					{
						//System.out.print(" #IMPLIED >\n");
						dtdBuilder.append(" #IMPLIED >\n");
					}
