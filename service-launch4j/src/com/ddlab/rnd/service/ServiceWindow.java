...
public class ServiceWindow 
{
	...
	private void createTray()
	{
		try 
		{
			img = new ImageIcon(AppConstants.configImgPath);
			trayIcon = new TrayIcon(img.getImage());
			tray = SystemTray.getSystemTray();
			popup = new PopupMenu();
			showItem = new MenuItem(AppConstants.SHOW_MENU_ITEM);
			exitItem = new MenuItem(AppConstants.EXIT_MENU_ITEM);
			popup.add(exitItem);
			popup.add(showItem);
	...
	private void addListeners()
	{
		...
		try 
		{
			...
			btnStartService.addActionListener( new ActionListener() {
				...
				{
					statusLbl.setText(AppConstants.SERVICE_STARTED_TXT);
					lblImage.setIcon(new ImageIcon(AppConstants.startImg));
			...
			btnStopService.addActionListener( new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) 
				{
					...
					lblImage.setIcon(new ImageIcon(AppConstants.stopImg));
	...
	private void initializeUI() 
	{
		...
		btnStartService = new JButton(AppConstants.START_SERVICE_TXT);
		btnStartService.setHorizontalAlignment(SwingConstants.LEFT);
		btnStartService.setIcon(new ImageIcon(AppConstants.buttonStartImgPath));
		btnStartService.setBounds(20, 188, 136, 37);
		frame.getContentPane().add(btnStartService);

		btnStopService = new JButton(AppConstants.STOP_SERVICE_TXT);
		btnStopService.setHorizontalAlignment(SwingConstants.LEFT);
		btnStopService.setIcon(new ImageIcon(AppConstants.buttonStopImgPath));
